...
int writeFile(const char* path, const char* dir)
{
    ...
    else
    {
        ...
        while (i < rem_n)
        {
            ...
            // file_name conterrà solo il nome del file
...
int readNFiles(int N, const char* dir)
{
    ...
    char message1 [MSG_SIZE];
    memset(message1,0,MSG_SIZE);
    if(readn(fd_s,message1,MSG_SIZE)==-1)
    {
        errno = EREMOTEIO;
        return -1;
    }
    char* save = NULL;
    char* token = strtok_r(message1,";",&save);
    if (strcmp(token,"-1") == 0)
    { //operazione fallita
        token = strtok_r(NULL,";",&save);
        errno = (int)strtol(token, NULL, 10);
